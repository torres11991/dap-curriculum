...
To take the work you have completed on the branch and merge it into master, first go through the `git add`, `git commit -m`, and `git push` process.

When this is complete go to your GitHub account and select '`pull request`' from the tabs across the top.

Create a new pull request then merge the <nameOfBranch> into  master/main.
